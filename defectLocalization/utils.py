...
def loadDataframe(db):
    bug_dataframe = pd.read_csv("./%s.csv" % db, encoding='utf-8', index_col=0,header=0)
...
def fetchAndSaveDataframe(db):
    ...
    classes = bug_dataframe.component_id.unique().tolist()

    bug_dataframe['class'] = pd.Series(bug_dataframe.component_id.map(lambda x: classes.index(x)))
    ...
    bug_dataframe.to_csv("./%s.csv" % db, encoding='utf-8',header = True)
...
def vectorizeDoc(dataframe):
    ...
    print("Test/Train split")
    train, test = train_test_split(dataframe, train_size=0.8, random_state=seed)
    y_train, y_test = train.component_id, test.component_id
